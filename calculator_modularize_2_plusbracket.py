...
def readBracketfirst(line, index): #read "("
    token = {'type': 'BRACKET1'}
    return token, index + 1

def readBracketend(line, index): #read ")"
    token = {'type': 'BRACKET2'}
    return token, index + 1
...
def tokenize(line):
    ...
    while index < len(line):
        if line[index].isdigit():
            ...
            (token, index) = readBracketfirst(line, index)
        elif line[index] == ')':
...
def evaluateAsterandSlash(tokens): #aster&slash
    temp = 0
    float(temp)
...
def evaluatePlusandMinus(tokens): #plus&minus
    answer = 0
    tokens.insert(0, {'type': 'PLUS'}) # Insert a dummy '+' token
    index = 1
    while index < len(tokens):
...
while True:
    ...
    tokensas = evaluateAsterandSlash(tokens)
    answer = evaluatePlusandMinus(tokensas)
    print "answer = %f\n" % answer
